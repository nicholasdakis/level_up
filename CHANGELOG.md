...
- Replaced the "Send Feedback" button in About the Developer with a "Contact Us" button that opens a dialog with preset topics (Report a Bug, Feature Request, General Feedback, Other) before opening the email app, prefilling the subject line based on the chosen topic
- Fixed meal section chevrons in food logging being misaligned when macro data was present next to the meal header, caused by the chevron being inside a flexible layout that shifted based on title length
...
- Added pr_history table to track each PR broken per exercise with pr_type (weight, reps, volume), old_value, new_value, and achieved_at; the log_workout Postgres function now inserts a row per PR type broken per exercise atomically as part of the workout log transaction
- Added GET /workout_pr_summary route that returns PR counts by type for a given date range, with the same free/premium 14-day cap as workout history
...
- Used RepaintBoundary for pulsing badges chips to optimize the Badges
...
- Exercise search results are now ranked by match quality: names starting with the query appear first, then word boundary matches, then substring matches anywhere in the name
- Redesigned the exercise picker filter chips to use a single bold label instead of a two-line category/value layout
- Removed the "Muscle:" and "Equipment:" prefixes from exercise row subtitles
